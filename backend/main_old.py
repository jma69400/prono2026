...
def fetch_match_results() -> dict:
    ...
    with get_db() as db:
        for m in api_matches:
            ...
            # === Détection de la PÉRIODE de match ===
            # On se base UNIQUEMENT sur les champs explicites de l'API :
            # - status='PAUSED' = mi-temps (l'API met "PAUSED" pendant la pause)
            # - score.duration = 'REGULAR' / 'EXTRA_TIME' / 'PENALTY_SHOOTOUT'
            # - minute permet de discriminer 1ère/2ème mi-temps en cas de duration='REGULAR'
            #
            # IMPORTANT : pas d'heuristique "halfTime score == fullTime score = mi-temps"
            # car ça produit des faux positifs (la 2ème mi-temps peut avoir 0 but, score
            # identique à la mi-temps mais on n'est PAS en mi-temps).
            if status in ("FINISHED", "AWARDED"):
                period_norm = "FULL_TIME"
            elif status == "PAUSED":
                # API renvoie PAUSED pour la mi-temps OU pause prolongations
                if period_api == "EXTRA_TIME":
                    period_norm = "EXTRA_TIME_PAUSE"
                else:
                ...
                # En IN_PLAY avec duration='REGULAR' : on distingue 1ère vs 2ème mi-temps via minute
                if minute is not None and str(minute).isdigit() and int(str(minute).split('+')[0]) > 45:
